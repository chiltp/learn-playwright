...
Notes from questions asked during the Playwright learning journey.
...
- `git commit` = save a snapshot on **your machine only**. Like filing a recipe card in your kitchen binder.
- `git push` = send your commits to **GitHub (remote server)**. Like sending the recipe to headquarters.
